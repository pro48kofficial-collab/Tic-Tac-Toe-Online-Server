...
(() => {
  ...
  if (!id) { id = (crypto && crypto.randomUUID) ? crypto.randomUUID() : 'p_' + Date.now() + '_' + Math.random().toString(36).slice(2); localStorage.setItem('ttt_id', id); }
  ...
  let n=0; const timer=setInterval(()=>{n=Math.min(100,n+4);$('bar').style.width=n+'%';$('percent').textContent=n+'%';if(n>=100){clearInterval(timer);$('loading').remove();$('app').hidden=false;renderProfile();}},30);
  connect();
})();
</script></body></html>`;
