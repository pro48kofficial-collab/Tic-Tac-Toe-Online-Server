...
(() => {
  ...
  if (!id) {
    try { id = (globalThis.crypto && globalThis.crypto.randomUUID ? globalThis.crypto.randomUUID() : null) || ('p_' + Date.now() + '_' + Math.random().toString(36).slice(2)); }
  ...
  document.querySelectorAll('.tab').forEach(function(b){ b.addEventListener('click', function(){ switchPage(b.dataset.page); }); });
  ...
  function renderGame(m) {
    ...
    const mine=m.players.find(function(p){return p.id===id;}), other=m.players.find(function(p){return p.id!==id;});
    ...
    $('status').textContent=m.result ? (m.result==='draw'?'🤝 Нічия!':\`🏆 Переміг \${esc(m.players.find(p=>p.mark===m.result)?.nickname || '')}\`) : (m.players.length<2?'⏳ Чекаємо суперника…':(m.turn===(mine && mine.mark)?'🔥 Твій хід':'⏳ Хід суперника'));
    $('board').style.gridTemplateColumns=\`repeat(\${m.size},1fr)\`;
    $('board').innerHTML=m.board.map((v,i)=>\`<button class="cell \${v==='X'?'x':v==='O'?'o':''}" \${v||m.result||m.players.length<2||m.turn!==(mine && mine.mark)?'disabled':''} onclick="window.makeMove(\${i})">\${v==='X'?'❌':v==='O'?'⭕':''}</button>\`).join('');
  ...
  $('create').onclick=()=>{$('modal').hidden=false}; $('cancel').onclick=()=>{$('modal').hidden=true}; $('modal').addEventListener('click',function(e){if(e.target.id==='modal')$('modal').hidden=true});
  ...
  // Усі обробники вже підключені після створення DOM.
  renderProfile();
  connect();
})();
</script></body></html>`;
