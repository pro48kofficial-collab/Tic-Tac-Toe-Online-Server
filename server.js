...
<div class="modal" id="joinModal" hidden><div class="sheet"><h2>🔒 Приватний сервер</h2><div class="field"><label>Пароль сервера</label><input class="input" id="joinPassword" type="password" inputmode="numeric" maxlength="12" placeholder="Введи пароль"></div><div id="joinRoomHint" style="color:#8991ad;font-size:12px;margin-bottom:8px"></div><div class="sheet-actions"><button class="ghost" id="joinCancel">Скасувати</button><button class="primary" id="joinConfirm">Приєднатися</button></div></div></div>
...
(() => {
  ...
  function connect() {
    ...
    ws.onopen = () => { send({type:'profile', nickname:profile.nickname || 'Гравець', avatar:profile.avatar || ''}); send({type:'servers'}); send({type:'leaderboard'}); const room = new URLSearchParams(location.search).get('room'); if (room) setTimeout(() => openPrivateJoin(room), 250); };
  ...
  function handle(m) {
    ...
    if (m.type === 'joined') { joinedRoom = true; showGame(); if (m.private && m.room) showShare(m.room, m.password); }
  ...
  function openPrivateJoin(room) {
    $('joinModal').hidden=false; $('joinPassword').value=''; $('joinRoomHint').textContent='Код сервера: '+room; $('joinPassword').focus();
    window.__privateRoom=room;
  }
  $('joinConfirm').onclick=()=>{ const room=window.__privateRoom; const password=$('joinPassword').value.trim(); if(!password){alert('Введи пароль.');return;} $('joinModal').hidden=true; send({type:'join',room,password}); };
  $('joinCancel').onclick=()=>{ $('joinModal').hidden=true; history.replaceState({},'',location.pathname); };
  ...
  function showShare(room, password) { const url=location.origin+'/?room='+encodeURIComponent(room); $('share').hidden=false; $('share').innerHTML=\`<b>🔒 Приватна кімната</b><div style="color:#8991ad;font-size:12px;margin-top:4px">Пароль: <strong style="color:white;font-size:16px">\${esc(password||'')}</strong></div><div style="color:#8991ad;font-size:12px;margin-top:6px">Надішли другу це посилання:</div><input class="input" id="shareInput" value="\${esc(url)}" readonly><button class="primary" style="width:100%;margin-top:8px" onclick="window.copyShare()">📋 Скопіювати</button>\`; }
...
(function(){
  ...
  function fallbackConnect(){
    ...
    socket.onopen=function(){
      ...
      send({type:'servers'}); send({type:'leaderboard'}); var room=new URLSearchParams(location.search).get('room'); if(room) setTimeout(function(){openPrivateJoinSafe(room);},250);
    };
    socket.onmessage=function(ev){
      ...
      if(m.type==='joined'){ if($('game')) $('game').hidden=false; document.body.style.overflow='hidden'; window.scrollTo(0,0); if(m.private && m.room) showShareSafe(m.room,m.password); }
      if(m.type==='state'){ renderGameSafe(m); }
      if(m.type==='roomClosed'){ if($('game')) $('game').hidden=true; document.body.style.overflow=''; alert('Сервер закрит.'); send({type:'servers'}); }
    };
    socket.onclose=function(){ socket=null; clearTimeout(timer); timer=setTimeout(fallbackConnect,1500); };
  }
  function openPrivateJoinSafe(room){
    if($('joinModal')){$('joinModal').hidden=false;$('joinPassword').value='';$('joinRoomHint').textContent='Код сервера: '+room;window.__privateRoomSafe=room;}
  }
  function showShareSafe(room,password){
    var url=location.origin+'/?room='+encodeURIComponent(room); if($('share')){$('share').hidden=false;$('share').innerHTML='<b>🔒 Приватна кімната</b><div style="color:#8991ad;font-size:12px;margin-top:4px">Пароль: <strong style="color:white;font-size:16px">'+String(password||'')+'</strong></div><div style="color:#8991ad;font-size:12px;margin-top:6px">Надішли другу це посилання:</div><input class="input" id="shareInput" value="'+String(url).replace(/"/g,'&quot;')+'" readonly><button class="primary" style="width:100%;margin-top:8px" onclick="window.copyShare()">📋 Скопіювати</button>';}}
  function renderProfileSafe(p){
    ...
    if($('avatarPreview')) $('avatarPreview').innerHTML=p.avatar ? '<img src=\"'+String(p.avatar).replace(/\"/g,'&quot;')+'\" alt=\"\">' : '🙂';
  }
  function handleAvatarFile(e){
    var f=e.target.files && e.target.files[0]; if(!f) return;
    if(f.size>3*1024*1024){alert('Фото завелике. Максимум 3 МБ.');e.target.value='';return;}
    var r=new FileReader();
    r.onload=function(){
      var img=new Image();
      img.onload=function(){
        var c=document.createElement('canvas'), s=256; c.width=c.height=s;
        var ctx=c.getContext('2d'), k=Math.min(s/img.width,s/img.height), w=img.width*k, h=img.height*k;
        ctx.clearRect(0,0,s,s); ctx.drawImage(img,(s-w)/2,(s-h)/2,w,h);
        var data=c.toDataURL('image/jpeg',0.82);
        var p={}; try{p=JSON.parse(localStorage.getItem('ttt_profile')||'{}')}catch(x){}
        p.avatar=data; localStorage.setItem('ttt_profile',JSON.stringify(p));
        renderProfileSafe(p);
      };
      img.onerror=function(){alert('Не вдалося відкрити це зображення. Спробуй інше фото.');};
      img.src=r.result;
    };
    r.onerror=function(){alert('Не вдалося прочитати файл.');};
    r.readAsDataURL(f);
  }
  function renderServers(list){
  ...
  function renderGameSafe(m){
    if(!$('game')) return;
    $('game').hidden=false;
    document.body.style.overflow='hidden';
    if($('gameName')) $('gameName').textContent=m.name||'Сервер';
    if($('gameInfo')) $('gameInfo').textContent=(m.size||3)+'×'+(m.size||3)+' • '+((m.players||[]).length)+'/2 гравців';
    var mine=(m.players||[]).find(function(p){return p.id===pid;});
    var other=(m.players||[]).find(function(p){return p.id!==pid;});
    function card(p){
      if(!p) return '<div class=\"player\"><div class=\"player-avatar\">⌛</div><div class=\"player-info\"><b>Очікуємо</b><small>Другий гравець</small></div></div>';
      var av=p.avatar?'<div class=\"player-avatar\"><img src=\"'+String(p.avatar).replace(/\"/g,'&quot;')+'\" alt=\"\"></div>':'<div class=\"player-avatar\">🙂</div>';
      return '<div class=\"player '+(m.turn===p.mark&&!m.result?'active':'')+'\">'+av+'<div class=\"player-info\"><b>'+String(p.nickname||'Гравець').replace(/[&<>]/g,'')+(p.id===pid?' (ти)':'')+'</b><small>'+String(p.rank||'')+'</small></div><div class=\"mark\">'+(p.mark==='X'?'❌':'⭕')+'</div></div>';
    }
    if($('players')) $('players').innerHTML=card(mine)+card(other);
    if($('status')) $('status').textContent=m.result?(m.result==='draw'?'🤝 Нічия!':'🏆 Переміг '+String(((m.players||[]).find(function(p){return p.mark===m.result;})||{}).nickname||'').replace(/[&<>]/g,'')):((m.players||[]).length<2?'⏳ Чекаємо суперника…':(mine&&m.turn===mine.mark?'🔥 Твій хід':'⏳ Хід суперника'));
    if($('board')){
      $('board').style.gridTemplateColumns='repeat('+(m.size||3)+',1fr)';
      $('board').innerHTML=(m.board||[]).map(function(v,i){
        var disabled=!!v||!!m.result||(m.players||[]).length<2||!mine||m.turn!==mine.mark;
        return '<button class=\"cell '+(v==='X'?'x':v==='O'?'o':'')+'\" '+(disabled?'disabled':'')+' data-move=\"'+i+'\">'+(v==='X'?'❌':v==='O'?'⭕':'')+'</button>';
      }).join('');
    }
    if($('again')){ $('again').disabled=!m.result||(m.players||[]).length<2; $('again').style.opacity=$('again').disabled?.55:1; }
  }
  ...
  function wire(){
    document.addEventListener('click',function(e){
      ...
      if(move){e.preventDefault();send({type:'move',index:Number(move.getAttribute('data-move'))});return;}
    ...
    if($('joinConfirm')) $('joinConfirm').onclick=function(e){e.preventDefault();var room=window.__privateRoomSafe||window.__privateRoom;var password=($('joinPassword').value||'').trim();if(!password){alert('Введи пароль.');return;}$('joinModal').hidden=true;send({type:'join',room:room,password:password});};
    if($('joinCancel')) $('joinCancel').onclick=function(e){e.preventDefault();$('joinModal').hidden=true;history.replaceState({},'',location.pathname);};
    ...
    if($('avatarFile')) $('avatarFile').onchange=handleAvatarFile;
    if($('saveProfile')) $('saveProfile').onclick=function(e){
      ...
      old.nickname=($('nickname').value||'Гравець').trim()||'Гравець'; localStorage.setItem('ttt_profile',JSON.stringify(old)); send({type:'profile',nickname:old.nickname,avatar:old.avatar||''}); renderProfileSafe(old); alert('Профіль збережено!');
    };
  ...
  ws.on('message', async raw => {
    ...
    try {
      ...
      if(m.type==='create') {
        ...
        const password = m.public ? '' : String(Math.floor(100000 + Math.random()*900000));
        const room={id:roomId,name:safeText(m.name,'Моя кімната',30),size,public:Boolean(m.public),password,board:Array(size*size).fill(''),turn:'X',result:null,players:[]};
        room.players.push({...p,ws,mark:'X'}); rooms.set(roomId,room); ws.room=roomId; ws.playerId=id; ws.mark='X';
        send(ws,{type:'joined',room:roomId,private:!room.public,password:room.password}); broadcast(room); return;
      }
      if(m.type==='join') {
        ...
        if(!room.public && String(m.password||'') !== String(room.password||'')) return send(ws,{type:'error',message:'Неправильний пароль сервера.'});
        ...
        send(ws,{type:'joined',room:room.id,private:!room.public,password:room.password}); broadcast(room); return;
      }
