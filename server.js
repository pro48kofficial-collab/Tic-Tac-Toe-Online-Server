...
(() => {
  ...
  // Loading screen is independent from WebSocket/database, so it can never get stuck.
  let n=0;
  const finishLoading=()=>{
    n=100;
    $('bar').style.width='100%';
    $('percent').textContent='100%';
    setTimeout(()=>{ $('loading').remove(); $('app').hidden=false; renderProfile(); }, 120);
  };
  const timer=setInterval(()=>{
    n=Math.min(100,n+5);
    $('bar').style.width=n+'%';
    $('percent').textContent=n+'%';
    if(n>=100){ clearInterval(timer); finishLoading(); }
  },40);
  setTimeout(()=>{ clearInterval(timer); finishLoading(); }, 1600);
  connect();
})();
</script></body></html>`;
